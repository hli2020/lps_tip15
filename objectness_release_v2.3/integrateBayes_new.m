function scoreBayes = integrateBayes_new(cues,score,params)
...
for cue_id = 1:length(cues)    
    ...
    switch upper(cues{cue_id})
        ...
        case 'SS'            
         ...
         case 'LS'                                           
            %combine LS cue using as posterior - score            
            likelihood{cue_id} = [1;1];   
...
for cue_id = 1:length(cues)
    ...
    switch upper(cues{cue_id})
        ...
        case 'LS'                                           
            binNumber{cue_id} = ones(size(score(:,cue_id)));
            posteriorLS = score(:,cue_id);
...
%if LS is a cue integrate it using the likelihood as posterior
scoreBayes = scoreBayes .* posteriorLS;
