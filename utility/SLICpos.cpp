...
{ 
	...
	for ( int j = 0; j< NumPixel; j++ ){		
		...
		meanSupP[ kk ] +=  (j-1)/wid + 1;     // row
		meanSupPCol[ kk ] += (j-1)%wid + 1;
	}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	plhs[5] = mxCreateDoubleMatrix( numlabels, 1, mxREAL);
	double * meanSupPos = (double *)mxGetPr(plhs[5]);            // row

	plhs[6] = mxCreateDoubleMatrix( numlabels, 1, mxREAL);
	double * meanSupPosCol = (double *)mxGetPr(plhs[6]);         // column
	...
	// vector <double> meanSupPos( numlabels );        // row
	// vector <double> meanSupPosCol( numlabels );     // column

	slic.DoMeanSup( rr, gg, bb, numlabels, NumPixel , outlabel, meanSuprr, meanSupgg, meanSupbb, meanSupPos, meanSupPosCol, width );	
